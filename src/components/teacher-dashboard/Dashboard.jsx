...
//   return (
//     <div className="flex items-center gap-2 rounded-lg bg-slate-200 p-2.5">
//       <span className="shrink-0 rounded-md bg-green-500 px-3 py-1.5 text-[11px] font-extrabold text-white">
//         {step}
//       </span>

//       <div className="h-12 flex items-center justify-center">
//         <span className="text-center text-lg leading-5 text-slate-600 sm:text-[13px] margin-top-1">
//           {text}
//         </span>
//       </div>
//     </div>
...
//             <div
//               key={index}
//               className={` ${isLastStep ? "" : "relative flex-1"}`}
//             >
//               {/* Step circle */}
...
//               {/* Connecting line */}
//               {!isLastStep && (
//                 <div className="absolute left-10 right-0 top-[15px] h-[2px] bg-blue-900" />
//               )}

//               {/* Step title */}
//               <small className="mt-1.5 block whitespace-nowrap text-[11px] font-bold text-slate-500">
//                 {item.step}
//               </small>
//             </div>
//           );
...
const lessons = [
  {
    number: "01.",
    title: "Road Test Preparation Course",
    text: "Focused training on test routes, examiner expectations, and mock driving tests.",
    progress: 20,
  },
  {
    number: "02.",
...
function StepItem({ step, text }) {
  return (
    <div className="flex min-h-16 items-center gap-2 rounded-lg bg-slate-200 p-2.5">
      <span className="shrink-0 rounded-md bg-green-500 px-3 py-1.5 text-[11px] font-extrabold text-white">
        {step}
      </span>

      <div className="flex min-w-0 flex-1 items-center ">
        <span className="text-center text-xs leading-5 text-slate-600 sm:text-[13px]">
          {text}
        </span>
      </div>
    </div>
...
function LessonCard({ lesson }) {
  return (
    <div className="rounded-xl bg-white p-3 shadow-sm">
      ...
      <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-slate-200">
        <div
          className="h-full rounded-full bg-blue-900 transition-[width] duration-300"
          style={{
...
function TableHead({ children }) {
  return (
    <th className="whitespace-nowrap px-3 py-2.5 text-[13px] font-extrabold sm:px-4">
      {children}
    </th>
  );
}
...
function TableData({ children, strong = false }) {
  return (
    <td
      className={`whitespace-nowrap px-3 py-2.5 text-[12px] sm:px-4 ${
        strong ? "font-bold text-slate-900" : "text-slate-500"
      }`}
    >
      {children}
